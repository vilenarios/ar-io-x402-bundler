...
export class DataItemOffsetsMigrator extends Migrator {
  constructor(private readonly knex: Knex) {
    super();
  }
  ...
  public migrate() {
    return this.operate({
      name: "migrate to data item offsets table",
      operation: async () => {
        await this.knex.schema.createTable("data_item_offsets", (table) => {
          table.string("data_item_id", 43).primary();
          table.string("root_bundle_id", 43).notNullable();
          table.bigInteger("start_offset_in_root_bundle").notNullable();
          table.bigInteger("raw_content_length").notNullable();
          table.integer("payload_data_start").notNullable();
          table.string("payload_content_type", 255).nullable();
          table.string("parent_data_item_id", 43).nullable();
          table
            .bigInteger("start_offset_in_parent_data_item_payload")
            .nullable();
          table.bigInteger("expires_at").nullable();
          table.timestamp("created_at").defaultTo(this.knex.fn.now());

          // Indexes for common queries
          table.index("root_bundle_id", "idx_root_bundle_id");
          table.index("parent_data_item_id", "idx_parent_data_item_id");
  ...
  public rollback() {
    return this.operate({
      name: "rollback from data item offsets table",
      operation: async () => {
        await this.knex.schema.dropTableIfExists("data_item_offsets");
      },
    });
  }
}

export class ConfigTableMigrator extends Migrator {
  constructor(private readonly knex: Knex) {
    super();
  }
  ...
  public migrate() {
    return this.operate({
      name: "migrate to config table",
      operation: async () => {
        await this.knex.schema.createTable("config", (table) => {
        ...
        await this.knex("config").insert([
          {
            key: "cleanup_fs_cursor",
            value: "0",
            description: "Filesystem cleanup cursor",
          },
          {
            key: "optical_api_key",
            value: process.env.OPTICAL_API_KEY || "",
            description: "Optical bridge API key",
          },
          {
            key: "optical_base_url",
            value: process.env.OPTICAL_BASE_URL || "",
            description: "Optical bridge URL",
          },
        ]);
      },
    });
  ...
  public rollback() {
    return this.operate({
      name: "rollback from config table",
      operation: async () => {
        await this.knex.schema.dropTableIfExists("config");
      },
    });
  }
}
