...
if (process.env.DATA_ITEM_BUCKET) {
  bucketNameToRegionMap[process.env.DATA_ITEM_BUCKET] =
    process.env.DATA_ITEM_BUCKET_REGION ?? s3Region;
}
...
function s3ClientForBucket(bucketName: string): S3Client {
  const region = bucketNameToRegionMap[bucketName] ?? s3Region;
...
export class S3ObjectStore implements ObjectStore {
  ...
  public async deleteObject(Key: string): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`Successfully deleted S3 object`, { Key });
    } catch (error) {
